<?php
// If the view request is set, fetch request details
...
$sql = "SELECT empid, empName, empMobile,empSpecial FROM technician_tb";
$resultTechnicians = $conn->query($sql); // Store the result in a different variable to avoid overwriting the previous result
...
if (isset($_REQUEST['Assign'])) {
    if (empty($_REQUEST['request_id']) || empty($_REQUEST['request_info']) || empty($_REQUEST['requestername']) || empty($_REQUEST['address1']) || empty($_REQUEST['address2']) || empty($_REQUEST['requestercity']) || empty($_REQUEST['requesterstate']) || empty($_REQUEST['requesterzip']) || empty($_REQUEST['requesteremail']) || empty($_REQUEST['requestermobile']) || empty($_REQUEST['assigntech']) || empty($_REQUEST['inputdate'])) {
        ...
        $sql = "INSERT INTO assignwork_tb (request_id, request_info, requester_name, requester_add1, requester_add2, requester_city, requester_state, requester_pin, requester_email, requester_mobile, assign_tech, assign_date) VALUES ('$rid', '$rinfo', '$rname', '$radd1', '$radd2', '$rcity', '$rstate', '$rpin', '$remail', '$rmobile', '$rtech', '$rdate')";
    ...
    <form action="" method="POST">
        ...
        <div class="form-row">
            ...
            <div class="form-group col-md-4">
                ...
                <input type="text" class="form-control" id="requesterzip" name="requesterzip" onkeypress="isInputNumber(event)" value="<?php if (isset($row['requester_pin'])) echo $row['requester_pin']; ?>">
            </div>
        </div>
            ...
            <select class="form-control" id="assigntech" name="assigntech">
                ...
                if ($resultTechnicians->num_rows > 0) {
                    while ($rowTechnician = $resultTechnicians->fetch_assoc()) {
                        $techDetails = $rowTechnician['empName'] . " - " . $rowTechnician['empMobile']. " - " . $rowTechnician['empSpecial'];
                        echo "<option value='" . $techDetails . "'>" . $techDetails . "</option>";
                    }
                } else {
                    echo "<option value=''>No Technician Available</option>";
                }
                ?>
            </select>
        </div>
